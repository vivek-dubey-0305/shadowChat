...
def start_server(connection):
    ...
    #Create a thread to continuously liusten for connections
...
def end_server(connection):
    ...
    #Alert all user that the server is closing
    ...
    while True:
        try:
            ...
            if client_address[0] in connection.banned_ips:
                ...
                #Send a message to recieve client info
                message_packet = create_message("INFO", "Admin (private)", "Please send your name", light_green)
                message_json = json.dumps(message_packet)
                client_socket.send(message_json.encode(connection.encoder))

                #Wait for confirmation messsage to be sent verifying the connectionm
...
def create_message(flag, name, message, color):
    '''Return a message packet ot be sent'''
    message_packet = {
        "flag": flag,
        "name": name,
        "message": message,
...
def process_message(connection, message_json, client_socket, client_address=(0,0)):
    ...
    if flag == "INFO":
        ...
        #Bradcast the new client is joining and update GUI
        message_packet = create_message("MESSAGE", "Admin (Broadcast)", f"{name} has joined hte serve!!", light_green)
        message_json = json.dumps(message_packet)
        broadcast_message(connection, message_json.encode(connection.encoder))

        #Update server UI
        client_listbox.insert(END, f"{name}         IP Addr: {client_address[0]}")

        #Now that a client has been established, start a thread to recieve message
        ...
        history_listbox.index(0, f"{name}: {message}")
        history_listbox.itemconfig(0, fg=color)

    elif flag == "DISCONNECT":
        #Close/remove the client socket
        ...
        #Alert all users that the client has lest the chat
...
def private_message(connection):
    ...
    #Select the client from the client listbox and access theor client socket
